<template>
  <scroll @scroll='scroll' :probeType='probeType' :listenScroll='listenScroll' class='listview' :data='data' ref='listview'>
    <ul>
    ...
    <div class="list-shortcut" @touchstart='onShortcut' @touchmove.stop.prevent='onShortctuTouchMove'>
      <ul >
...
export default {
  ...
  watch: {
    ...
    scrollY (newY) {
      ...
      for (let [i, v] of listHeight.entries()) {
        if (i >= listHeight.length - 1) return
        let height1 = v
  ...
  methods: {
    ...
    _scrollTo (index, type = false) {
      ...
      this.$refs.listview.scrollToElement(this.$refs.listGroup[index], type ? 0 : 200)
    },
    _caculateHeight () {
      this.listHeight = []
